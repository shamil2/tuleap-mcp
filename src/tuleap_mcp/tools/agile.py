...
async def _get_tracker_id_by_name(client: TuleapClient, project_id: int, tracker_name: str) -> Optional[int]:
    ...
    for t in trackers:
        ...
        if tracker_name.lower() in t.get("item_name", "").lower() or tracker_name.lower() in t.get("name", "").lower() or t.get("shortname", "").lower() == tracker_name.lower():
            return t.get("id")
    return None
...
async def get_user_stories(client: TuleapClient, project_id: int, epic_id: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    if not tracker_id:
        raise Exception(f"Could not find a 'User Story' tracker in project {project_id}")
